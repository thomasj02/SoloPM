...
INSTRUCTIONS = (
    ...
    "projects and declines whenever ownership is ambiguous. Remote projects "
    "(`github_repo` set — the repo's checkout lives with you, not with the backend) use "
    "the same move-with-branch call: over the HTTP MCP or CLI the SoloPM client pushes "
    "the branch from your machine, and the backend runs the PR lifecycle through the "
    "GitHub API. Only in a stdio (in-process) session is there no client push half — "
    "there, pushing the ticket branch yourself before the move is the one sanctioned "
    "exception to the no-push rule.\n\n"
